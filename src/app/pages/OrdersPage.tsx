...
export default function OrdersPage() {
  ...
  useEffect(() => {
    ...
    if (accessToken && user) {
      console.log('🔄 [OrdersPage] Authenticated, loading orders...', {
      ...
      console.log('⏳ [OrdersPage] Waiting for authentication...', {
        hasToken: !!accessToken,
        hasUser: !!user
      });
      setIsLoading(false);
  ...
  const loadOrders = async () => {
    try {
      ...
      if (!accessToken || !user) {
        console.log('ℹ️ [OrdersPage] User not authenticated, skipping load orders');
        setIsLoading(false);
        return;
      }

      console.log('📡 [OrdersPage] Calling API /orders with token...');
  ...
  const handlePay = async (order: Order, e: React.MouseEvent) => {
    ...
    console.log('🔍 Paying for order:', order.id, 'Amount:', order.totalAmount);
    ...
    try {
      ...
      if (response.ok) {
        ...
        refreshIntervals.forEach((delay) => {
          setTimeout(() => {
            console.log(`🔄 Force refreshing orders (${delay}ms)...`);
            loadOrders();
          }, delay);
        });
          ...
          <Card className="p-6 hover:shadow-lg transition cursor-pointer">
            ...
            <div className="space-y-3 mb-4">
              {order.items.slice(0, 2).map((item, index) => {
                ...
                console.log(`📦 Order ${order.id} Item ${index}:`, {
