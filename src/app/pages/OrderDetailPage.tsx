...
export default function OrderDetailPage() {
  ...
  const loadRefund = async () => {
    ...
    try {
      console.log('💰 [LoadRefund] Starting for order ID:', id);
      ...
      console.log('💰 [LoadRefund] Response status:', response.status);

      if (response.ok) {
        const result = await response.json();
        console.log('💰 [LoadRefund] Response data:', result);
        
        if (result.success && result.refund) {
          console.log('💰 [LoadRefund] Refund found:', {
            id: result.refund.id,
          ...
          console.log('💰 [LoadRefund] No refund exists for order:', id);
          setRefund(null);
        }
      } else {
  ...
  const handleConfirmShipment = async () => {
    ...
    try {
      console.log('🔄 [ConfirmShipment] Starting...', { refundId: refund.id, status: refund.status });
      ...
      console.log('📡 [ConfirmShipment] Response status:', response.status, response.statusText);

      const result = await response.json();
      console.log('📦 [ConfirmShipment] Response data:', result);
  ...
  const handlePaymentConfirm = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.order) {
          console.log('✅ Updating local order state with:', result.order);
        ...
        refreshIntervals.forEach((delay) => {
          setTimeout(() => {
            console.log(`🔄 Force refreshing order data (${delay}ms)...`);
            loadOrder();
          }, delay);
        });
      } else {
      ...
      <div className="flex-1 bg-gray-50 py-8">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <Card className="p-6 mb-6">
            ...
            {console.log('🔍 [OrderDetail] Order items length:', order.items?.length || 0)}
