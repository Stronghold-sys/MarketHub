...
export default function AdminOrders() {
  ...
  const loadOrders = async () => {
    if (!user?.role || user.role !== 'admin') {
      console.log('[AdminOrders] User is not admin');
      console.log('[AdminOrders] User role:', user?.role || 'UNKNOWN');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      console.log('🔄 [AdminOrders] Refreshing session to ensure fresh token...');
      const refreshResult = await refreshSession();
      
      if (refreshResult.success && refreshResult.session) {
        console.log('✅ [AdminOrders] Session refreshed successfully');
      ...
      if (allOrders && allOrders.length >= 0) {
        setOrders(allOrders);
        console.log('✅ [AdminOrders] Loaded orders successfully:', allOrders.length);
      }
    } catch (error: any) {
