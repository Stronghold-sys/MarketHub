...
export default function AdminUsers() {
  ...
  useEffect(() => {
    ...
    if (accessToken && currentUser) {
      console.log('[AdminUsers] Auth state ready!');
      setIsWaitingForAuth(false);
      clearTimeout(timer);
    }
  ...
  const loadUsers = useCallback(async () => {
    ...
    console.log('[AdminUsers] Loading users from Supabase Auth...');
    console.log('[AdminUsers] Current user:', currentUser?.email, currentUser?.role);
    console.log('[AdminUsers] Access token:', accessToken ? `${accessToken.substring(0, 20)}...` : 'NO TOKEN');
    setIsLoading(true);
    
    try {
      const response = await api.get('/admin/users', accessToken!);
      if (response.success) {
        console.log(`✅ Loaded ${response.count} users from ${response.source}`);
  ...
  useEffect(() => {
    if (accessToken && currentUser) {
      console.log('[AdminUsers] accessToken available, loading users...');
      loadUsers();
    } else {
  ...
  const handleUpdateRole = async () => {
    ...
    console.log(`[AdminUsers] Updating role for user ${selectedUser.email} from ${selectedUser.role} to ${newRole}`);
    setIsLoading(true);
    ...
    try {
      ...
      if (response.success) {
        ...
        if (currentUser && currentUser.email === selectedUser.email) {
          console.log('[AdminUsers] Current user role updated, reloading session...');
  ...
  const handleUpdateStatus = async () => {
    ...
    console.log(`[AdminUsers] Updating status for user ${selectedUser.email} from ${selectedUser.status} to ${newStatus}`);
    setIsLoading(true);
  ...
  const handleBanUser = async (type: 'suspend' | 'ban', reason: string, duration: number, unit: string) => {
    ...
    console.log(`[AdminUsers] ${type}ing user:`, selectedUser);
    console.log(`[AdminUsers] User ID:`, selectedUser.id);
    console.log(`[AdminUsers] User email:`, selectedUser.email);
    console.log(`[AdminUsers] Duration: ${duration} ${unit}`);
    console.log(`[AdminUsers] Reason:`, reason);
    ...
    try {
      ...
      console.log('[AdminUsers] Ban response:', response);

      if (response.success) {
        console.log('✅ User banned in backend successfully');
  ...
  const handleUnbanUser = async () => {
    ...
    console.log(`[AdminUsers] Unbanning user ${selectedUser.email}`);
    ...
    try {
      ...
      if (response.success) {
        console.log('✅ User unbanned in backend successfully');
  ...
  const handleDeleteUser = async () => {
    ...
    console.log(`[AdminUsers] Deleting user ${selectedUser.email}`);
    setIsDeleting(true);
    ...
    try {
      ...
      if (response.success) {
        console.log('✅ User deleted in backend successfully');
