...
export default function AdminVouchers() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      console.log('🔄 [AdminVouchers] Auto-refreshing vouchers and stats...');
  ...
  const fetchVouchers = async () => {
    ...
    console.log('📋 [AdminVouchers] Fetching vouchers...');
    console.log('🔑 [AdminVouchers] Access token:', accessToken ? 'Present' : 'Missing');
    console.log('🔑 [AdminVouchers] Access token value:', accessToken?.substring(0, 20) + '...');
    console.log('🔑 [AdminVouchers] Public anon key:', publicAnonKey ? 'Present' : 'Missing');
    console.log('🌐 [AdminVouchers] Project ID:', projectId);
    
    const url = `https://${projectId}.supabase.co/functions/v1/make-server-adb995ba/vouchers`;
    console.log('🌐 [AdminVouchers] URL:', url);
    ...
    try {
      ...
      console.log('📡 [AdminVouchers] Response status:', response.status);
      console.log('📡 [AdminVouchers] Response headers:', Object.fromEntries(response.headers.entries()));
      
      // Get response text first
      const responseText = await response.text();
      console.log('📡 [AdminVouchers] Response text:', responseText);
      
      let data;
      try {
        data = JSON.parse(responseText);
        console.log('📡 [AdminVouchers] Response data:', data);
      } catch (parseError) {
      ...
      if (response.ok) {
        console.log('✅ [AdminVouchers] Vouchers loaded:', data.vouchers?.length || 0);
        console.log('✅ [AdminVouchers] First voucher:', data.vouchers?.[0]);
  ...
  const fetchStats = async () => {
    console.log('📊 [AdminVouchers] Fetching stats...');
    try {
      // ✅ CRITICAL FIX: Add Authorization header with publicAnonKey for Supabase Edge Functions
      ...
      if (response.ok) {
        ...
        console.log('✅ [AdminVouchers] Stats loaded:', data.stats);
        setStats(data.stats);
      } else {
  ...
  const confirmDeleteVoucher = async () => {
    ...
    console.log('🗑️ [AdminVouchers] Deleting voucher:', voucherId);
    ...
    try {
      ...
      if (response.ok) {
        console.log('✅ [AdminVouchers] Voucher deleted successfully');
