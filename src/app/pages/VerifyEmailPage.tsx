...
export default function VerifyEmailPage() {
  ...
  const data = useMemo(() => {
    ...
    if (location.state?.email && location.state?.verificationCode) {
      console.log('✅ Using verification data from location.state');
      return location.state;
    }

    // ✅ FIXED: Fallback to verification store instead of sessionStorage
    if (verificationData && isVerificationValid()) {
      console.log('✅ Using verification data from verification store');
      return {
    ...
    console.log('❌ No valid verification data found');
    return null;
  ...
  useEffect(() => {
    ...
    console.log('🔍 [VerifyEmailPage] Location state:', location.state);
    console.log('🔍 [VerifyEmailPage] Verification store:', verificationData);
    console.log('🔍 [VerifyEmailPage] Parsed data:', {
      email,
      name,
    ...
    if (email && verificationCode) {
      console.log('✅ [VerifyEmailPage] All verification data present and valid');
    } else {
  ...
  const handleVerify = async (codeString?: string) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        console.log('🗑️ Verification successful - data will be cleared from memory');
