...
export default function CheckoutPage() {
  ...
  console.log('🔍 CheckoutPage state - isProcessing:', isProcessing, 'items.length:', items.length);
  ...
  useEffect(() => {
    if (user?.addresses && user.addresses.length > 0 && !selectedAddress) {
      ...
      console.log('✅ [CheckoutPage] Default address initialized:', defaultAddr);
    }
  ...
  useEffect(() => {
    ...
    if (!hasInitialItemsRef.current && items.length === 0) {
      console.log('⚠️ Redirecting to cart: No items on initial load');
  ...
  useEffect(() => {
    console.log('🔍 CheckoutPage - showQRISModal changed to:', showQRISModal);
    console.log('🔍 CheckoutPage - qrCode:', qrCode);
    console.log('🔍 CheckoutPage - currentOrderId:', currentOrderId);
  ...
  useEffect(() => {
    ...
    if (currentOrderId) {
      ...
      if (status === 'paid' && accessToken) {
        ...
        (async () => {
          try {
            console.log('🔄 Updating payment status to paid - order remains waiting_payment...');
            ...
            if (response.ok) {
              ...
              console.log('✅ Payment status updated to paid:', result);
  ...
  const handlePaymentStatusChange = useCallback(async (status: PaymentStatus) => {
    console.log('💰 handlePaymentStatusChange called with status:', status);
    ...
    if (currentOrderId) {
      ...
      if (status === 'paid' && accessToken) {
        try {
          console.log('🔄 Updating payment status to paid and order to processing...');
          const response = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-adb995ba/orders/${currentOrderId}/payment-status`,
            {
              method: 'PUT',
          ...
          if (response.ok) {
            const result = await response.json();
            console.log('✅ Payment and order status updated:', result);
      ...
      // Add notification
      if (status === 'paid' && user) {
        console.log('🔔 [Checkout] Adding payment success notification for user:', user.id);
        const userNotif = {
          id: `notif-${Date.now()}`,
          userId: user.id,
        ...
        console.log('🔔 [Checkout] User notification object:', userNotif);
        addNotification(userNotif);

        // Also add notification for admin
        console.log('🔔 [Checkout] Adding admin notification');
        const adminNotif = {
        ...
        console.log('🔔 [Checkout] Admin notification object:', adminNotif);
  ...
  const handleAddAddress = async (newAddress: Address) => {
    ...
    try {
      ...
      try {
        ...
        console.log('✅ Addresses updated on backend:', data);
        ...
        if (profileResponse.ok) {
          ...
          if (profileData.success && profileData.user) {
            console.log('✅ User profile reloaded from server:', profileData.user);
            ...
            if (savedAddress) {
              ...
              console.log('✅ Selected address updated:', savedAddress);
            }
  ...
  const handleCheckout = async () => {
    ...
    if (isUnderMaintenance()) {
      console.log(' [Checkout] BLOCKED - System is under maintenance');
    ...
    try {
      ...
      const newOrder = {
        ...
        items: items.map(item => {
          ...
          console.log(`📊 Item ${item.productId}: product.price=${item.product?.price}, item.price=${item.price}, final=${itemPrice}`);
          console.log(`📷 Item ${item.productId} images:`, item.product?.images || item.product?.image || 'NO IMAGES');
      ...
      console.log('💾 Saving order to local store:', newOrder);
      console.log('📊 Order totals - subtotal:', newOrder.subtotal, 'shipping:', newOrder.shippingCost, 'total:', newOrder.totalAmount);
      addOrder(newOrder);
      ...
      console.log('💾 Saving payment to local store:', newPayment);
      ...
      console.log('ℹ️ Cart will be cleared after payment modal is closed or payment succeeds');
      ...
      setTimeout(() => {
        if (paymentType === 'qris') {
          ...
          console.log('✅ Opening E-Wallet Modal');
          setShowEWalletModal(true);
        } else if (paymentType === 'bank_transfer') {
          console.log('✅ Opening Virtual Account Modal');
          setShowVAModal(true);
        } else if (paymentType === 'cod') {
          console.log('✅ COD - Direct redirect to payment status');
          
          // ✅ ADD NOTIFICATION FOR COD ORDER
          if (user) {
            console.log('🔔 [Checkout] Adding COD notification for user:', user.id);
            ...
            console.log(' [Checkout] User COD notification object:', userCODNotif);
  ...
  return (
    <Layout>
      ...
      <QRISModal
        ...
        onClose={() => {
          console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
          console.log('🚪 QRIS Modal onClose called');
          ...
          console.log('🗑️ Closing modal - clearing cart');
          clearCart();
          
          if (currentPaymentStatus === 'paid') {
            console.log(' Payment successful, navigating to payment-status');
            navigate(`/payment-status?orderId=${currentOrderId}`);
          } else {
            console.log('⚠️ Payment not completed, navigating to orders');
            navigate('/orders');
          }
        }}
        qrCode={qrCode}
