...
export default function ResetPasswordPage() {
  ...
  const getResetData = () => {
    ...
    if (location.state?.email && location.state?.verificationCode) {
      console.log('✅ Using reset data from location.state');
    ...
    if (stored) {
      try {
        ...
        if (age < 15 * 60 * 1000) {
          console.log('✅ Using reset data from sessionStorage');
          return data;
        } else {
          console.log('⏰ Stored reset data expired');
    ...
    console.log('❌ No valid reset data found');
    return null;
  };
  ...
  useEffect(() => {
    ...
    console.log('🔍 [ResetPasswordPage] SessionStorage:', sessionStorage.getItem('pendingReset'));
    console.log('🔍 [ResetPasswordPage] Parsed data:', {
      email,
      name,
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        sessionStorage.removeItem('pendingReset');
        console.log('🗑️ Cleared reset data from sessionStorage');
