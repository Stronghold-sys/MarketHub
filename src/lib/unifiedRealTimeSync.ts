...
export function useCartRealTimeSync() {
  ...
  const syncCartToBackend = useCallback(async () => {
    ...
    try {
      ...
      console.log('🔄 [CartSync] Syncing cart to backend...', items.length, 'items');
  ...
  useEffect(() => {
    ...
    const loadCart = async () => {
      try {
        console.log('📥 [CartSync] Loading cart from backend...');
        ...
        if (data.success && data.cart) {
          console.log('✅ [CartSync] Cart loaded:', data.cart.length, 'items');
          setItems(data.cart);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    console.log('👂 [CartSync] Setting up Realtime listener for user:', user.id);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('🔔 [CartSync] Realtime update received:', payload);
          
          // Reload cart from backend
          if (payload.eventType === 'UPDATE' || payload.eventType === 'INSERT') {
            const newCart = payload.new as any;
            if (newCart.items) {
              console.log('✅ [CartSync] Updating local cart from Realtime');
    ...
    return () => {
      console.log('👋 [CartSync] Cleaning up Realtime listener');
...
export function useUserStatusCheck() {
  ...
  const checkUserStatus = useCallback(async () => {
    ...
    try {
      ...
      // console.log('🔍 [UserStatus] Checking user status...');
      ...
      if (response.status === 401) {
        ...
        // console.log('ℹ️ [UserStatus] Token expired or invalid (expected behavior)');
        return;
      }
      ...
      if (response.status === 403) {
        ...
        try {
          ...
          if (errorData.banned && errorData.banData) {
            console.log('🚫 [UserStatus] User is banned:', errorData.banData);
            setBanNotification({
              isBanned: true,
          ...
          if (errorData.deleted) {
            console.log('🗑️ [UserStatus] User was deleted');
          ...
          // console.log('ℹ️ [UserStatus] User account inactive (expected behavior)');
        } catch (parseError) {
          // If we can't parse the response, just log and continue
          // console.log('ℹ️ [UserStatus] Access forbidden (expected behavior)');
        }
        return;
      }
      ...
      // console.log('✅ [UserStatus] User status OK');
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    console.log('⏰ [UserStatus] Starting periodic status check');
    ...
    return () => {
      console.log('👋 [UserStatus] Stopping periodic check');
  ...
  useEffect(() => {
    ...
    console.log('👂 [UserStatus] Setting up Realtime listener for user status');
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('🔔 [UserStatus] Realtime update received:', payload);
    ...
    return () => {
      console.log('👋 [UserStatus] Cleaning up Realtime listener');
...
export function useOrderRealTimeSync() {
  ...
  useEffect(() => {
    ...
    console.log('👂 [OrderSync] Setting up Realtime listener for orders');
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('🔔 [OrderSync] Realtime update received:', payload);

          if (payload.eventType === 'UPDATE') {
            const updatedOrder = payload.new as any;
            console.log('✅ [OrderSync] Order status updated:', updatedOrder.id, '→', updatedOrder.status);
          ...
          if (payload.eventType === 'INSERT') {
            console.log('✅ [OrderSync] New order created');
    ...
    return () => {
      console.log('👋 [OrderSync] Cleaning up Realtime listener');
      supabase.removeChannel(channel);
    };
  }, [user, setOrders]);
}
...
export function useNotificationRealTimeSync() {
  ...
  useEffect(() => {
    ...
    console.log('👂 [NotificationSync] Setting up Realtime listener');
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('🔔 [NotificationSync] New notification received:', payload);
    ...
    return () => {
      console.log('👋 [NotificationSync] Cleaning up Realtime listener');
      supabase.removeChannel(channel);
    };
  }, [user]);
}
...
export function useRealTimeSync() {
  console.log('🔄 [RealTimeSync] Initializing all real-time features');
  ...
  console.log('✅ [RealTimeSync] All real-time features active');
}