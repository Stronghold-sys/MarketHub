...
export const authStoreHelpers = {
  ...
  setToken: (accessToken: string | null) => {
    ...
    console.log('✅ [AuthStore] Access token updated in memory');
  },
  logout: async () => {
    ...
    console.log('✅ [AuthStore] State cleared - UI should update now');
    ...
    try {
      ...
      if (result.success && !(result as any).aborted) {
        console.log('✅ [AuthStore] Logged out from Supabase Auth successfully');
  ...
  restoreSession: async () => {
    try {
      ...
      if (currentState.user && currentState.accessToken) {
        console.log('⏭️ [AuthStore] Valid session already in store, skipping Supabase check');
        return true;
      }

      console.log('🔄 [AuthStore] Attempting to restore session from Supabase Auth...');
      ...
      if (session && session.user) {
        ...
        console.log('✅ [AuthStore] Session restored from Supabase Auth:', user.email);
        ...
        console.log('ℹ️ [AuthStore] No active session found in Supabase Auth');
        return false;
      }
    } catch (error) {
