...
export function useProducts() {
  ...
  console.log('🔄 [useProducts] Hook called - products:', products.length, 'hasLoaded:', hasLoadedOnMount);
  ...
  const reloadProducts = useCallback(async () => {
    ...
    try {
      ...
      console.log('✅ [useProducts] Products reloaded successfully');
    } catch (err: any) {
  ...
  useEffect(() => {
    ...
    if (!hasLoadedOnMount && products.length === 0) {
      console.log('📦 [useProducts] Auto-loading products on mount...');
