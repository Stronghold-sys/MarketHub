...
console.log('📦 [Supabase] Loading supabase.ts v17.7.0 - ADDED AUTH ENDPOINTS TO PUBLIC');
...
export const getSupabase = (): SupabaseClient => {
  ...
  if (typeof window !== 'undefined' && window.__supabaseClient) {
    console.log('♻️ [Supabase] Reusing existing singleton client');
    return window.__supabaseClient;
  }
  
  console.log('🔧 [Supabase] Creating new Supabase client (SINGLETON)...');
  ...
  if (typeof window !== 'undefined') {
    ...
    console.log('✅ [Supabase] Client stored in window.__supabaseClient for persistence');
  }
...
export const logoutSupabase = async () => {
  try {
    console.log('🔐 [Supabase] Logging out from Supabase Auth...');
    ...
    console.log('✅ [Supabase] Logged out successfully from Supabase Auth');
    return { success: true };
  } catch (error: any) {
...
const apiCall = async (
  ...
  if (accessToken) {
    ...
    console.log(`✅ [API ${method}] Token added to X-Session-Token header`);
  } else {
    // ✅ FIX v17.6: Only show warning for non-public endpoints
    if (!isPublicEndpoint(endpoint)) {
      console.warn(`⚠️ [API ${method}] NO TOKEN provided for: ${endpoint}`);
    } else {
      console.log(`📖 [API ${method}] Public endpoint - no token required: ${endpoint}`);
    }
  }
  ...
  if (!response.ok) {
    ...
    if (errorData && (errorData.deleted || errorData.banned)) {
      console.log('🚨 [API] Returning ban/delete error data without throwing');
