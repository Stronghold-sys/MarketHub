...
export const useMaintenanceStore = create<MaintenanceStore>((set, get) => ({
  ...
  fetchMaintenanceStatus: async () => {
    try {
      ...
      if (response.success && response.maintenance) {
        console.log('🔧 [MaintenanceStore] Status fetched:', response.maintenance);
        set({ 
  ...
  setMaintenanceMode: async (enabled: boolean, message?: string, duration?: number, scheduledDate?: string, scheduledTime?: string) => {
    try {
      ...
      console.log('🔧 [MaintenanceStore] Setting maintenance:', { enabled, message, duration, scheduledDate, scheduledTime });
      ...
      console.log('🔑 [MaintenanceStore] Sending request with access token');
      ...
      if (response.success && response.maintenance) {
        console.log('✅ [MaintenanceStore] Maintenance mode updated:', response.maintenance);
        set({ 
          maintenance: response.maintenance,
          isLoading: false 
        });
...
export function useMaintenanceSync() {
  ...
  React.useEffect(() => {
    console.log('🔧 [MaintenanceSync] Initializing...');
    ...
    const channel = supabase.channel('maintenance-updates')
      .on(
        ...
        (payload) => {
          console.log('🔄 [MaintenanceSync] Realtime update received:', payload);
      ...
      .subscribe((status) => {
        console.log('🔧 [MaintenanceSync] Subscription status:', status);
      });

    return () => {
      console.log('🔧 [MaintenanceSync] Cleaning up subscription...');
