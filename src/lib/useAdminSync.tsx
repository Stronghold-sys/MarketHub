...
export function useAdminSync(intervalMs: number = 60000) {
  ...
  useEffect(() => {
    ...
    if (user?.role !== 'admin') {
      console.log('[useAdminSync] User is not admin, skipping auto-sync');
      return;
    }

    console.log('[useAdminSync] Starting admin data auto-sync...');
    ...
    const cleanup = () => {
      if (intervalRef.current) {
        ...
        console.log('[useAdminSync] Stopped admin data auto-sync');
      }
    };
    ...
    const performInitialSync = async () => {
      if (!isInitializedRef.current) {
        console.log('[useAdminSync] Performing initial sync...');
        setIsLoading(true);
        ...
        try {
          ...
          console.log('[useAdminSync] Initial sync completed');
        } catch (error) {
    ...
    intervalRef.current = setInterval(async () => {
      ...
      try {
        console.log('[useAdminSync] Syncing admin data...');
        setIsLoading(true);
        await syncAdminData();
        setLastSyncTime(new Date());
        console.log('[useAdminSync] Admin data synced successfully');
      } catch (error) {
