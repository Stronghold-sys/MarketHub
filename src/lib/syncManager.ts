...
async function apiRequest(
  ...
  if (import.meta.env.DEV && !isPublicEndpoint) {
    if (!token) {
      ...
      console.log(`🔑 [apiRequest] Token available for ${endpoint} (${token.substring(0, 20)}...)`);
    }
  }
...
export const ProductSync = {
  ...
  async loadProducts(): Promise<void> {
    console.log('🔄 [ProductSync] Loading products...');
    try {
      const data = await apiRequest('/products', { method: 'GET' });

      if (data.success && data.products) {
        productStoreHelpers.setProducts(data.products);
        console.log(`✅ [ProductSync] Loaded ${data.products.length} products`);
      } else {
  ...
  async getProduct(id: string): Promise<any> {
    console.log(`🔄 [ProductSync] Getting product ${id}...`);
    try {
      const data = await apiRequest(`/products/${id}`, { method: 'GET' });

      if (data.success && data.product) {
        console.log(`✅ [ProductSync] Got product ${id}`);
        return data.product;
      } else {
  ...
  async createProduct(productData: any): Promise<any> {
    console.log('🔄 [ProductSync] Creating product...');
    try {
      ...
      if (data.success && data.product) {
        ...
        console.log(`✅ [ProductSync] Created product ${data.product.id}`);
        return data.product;
      } else {
  ...
  async updateProduct(id: string, updates: any): Promise<any> {
    console.log(`🔄 [ProductSync] Updating product ${id}...`);
    try {
      ...
      if (data.success && data.product) {
        ...
        await this.loadProducts();
        console.log(`✅ [ProductSync] Updated product ${id}`);
        return data.product;
      } else {
  ...
  async deleteProduct(id: string): Promise<void> {
    console.log(`🔄 [ProductSync] Deleting product ${id}...`);
    try {
      ...
      if (data.success) {
        ...
        await this.loadProducts();
        console.log(`✅ [ProductSync] Deleted product ${id}`);
      } else {
  ...
  async uploadImage(file: File): Promise<string> {
    console.log('📤 [ProductSync] Uploading image...');
    try {
      ...
      if (data.success && data.url) {
        console.log(`✅ [ProductSync] Image uploaded: ${data.url}`);
        return data.url;
      } else {
        throw new Error('Upload failed');
      }
...
export const OrderSync = {
  ...
  async createOrder(orderData: any): Promise<any> {
    console.log('🔄 [OrderSync] Creating order...');
    try {
      ...
      if (data.success && data.order) {
        console.log(`✅ [OrderSync] Created order ${data.order.id}`);
        return data.order;
      } else {
  ...
  async getUserOrders(): Promise<any[]> {
    console.log('🔄 [OrderSync] Getting user orders...');
    try {
      const data = await apiRequest('/orders', { method: 'GET' });

      if (data.success && data.orders) {
        console.log(`✅ [OrderSync] Got ${data.orders.length} orders`);
        return data.orders;
      } else {
  ...
  async getAllOrders(): Promise<any[]> {
    console.log('🔄 [OrderSync] Getting all orders (admin)...');
    try {
  ...
  async getOrder(orderId: string): Promise<any> {
    console.log(`🔄 [OrderSync] Getting order ${orderId}...`);
    try {
      const data = await apiRequest(`/orders/${orderId}`, { method: 'GET' });

      if (data.success && data.order) {
        console.log(`✅ [OrderSync] Got order ${orderId}`);
        return data.order;
      } else {
  ...
  async updateOrderStatus(orderId: string, status: string, note?: string): Promise<any> {
    console.log(`🔄 [OrderSync] Updating order ${orderId} to ${status}...`);
    try {
      ...
      if (data.success && data.order) {
        console.log(`✅ [OrderSync] Updated order ${orderId} to ${status}`);
        return data.order;
      } else {
  ...
  async cancelOrder(orderId: string, reason: string): Promise<any> {
    console.log(`🔄 [OrderSync] Cancelling order ${orderId}...`);
    try {
...
export const RefundSync = {
  ...
  async createRefund(refundData: any): Promise<any> {
    console.log('🔄 [RefundSync] Creating refund...');
    try {
      ...
      if (data.success && data.refund) {
        console.log(`✅ [RefundSync] Created refund ${data.refund.id}`);
        return data.refund;
      } else {
  ...
  async getUserRefunds(): Promise<any[]> {
    console.log('🔄 [RefundSync] Getting user refunds...');
    try {
      const data = await apiRequest('/refunds', { method: 'GET' });

      if (data.success && data.refunds) {
        console.log(`✅ [RefundSync] Got ${data.refunds.length} refunds`);
        return data.refunds;
      } else {
  ...
  async getAllRefunds(): Promise<any[]> {
    console.log('🔄 [RefundSync] Getting all refunds (admin)...');
    try {
  ...
  async updateRefundStatus(refundId: string, status: string, adminNote?: string): Promise<any> {
    console.log(`🔄 [RefundSync] Updating refund ${refundId} to ${status}...`);
    try {
      ...
      if (data.success && data.refund) {
        console.log(`✅ [RefundSync] Updated refund ${refundId} to ${status}`);
        return data.refund;
      } else {
...
export const UserSync = {
  ...
  async getCurrentUser(): Promise<any> {
    console.log('🔄 [UserSync] Getting current user...');
    try {
      ...
      if (data.success && data.user) {
        ...
        console.log(`✅ [UserSync] Got current user ${data.user.id}`);
        return data.user;
      } else {
  ...
  async updateProfile(updates: any): Promise<any> {
    console.log('🔄 [UserSync] Updating profile...');
    try {
      ...
      if (data.success && data.user) {
        ...
        authStoreHelpers.setUser(data.user);
        console.log(`✅ [UserSync] Updated profile`);
        return data.user;
      } else {
  ...
  async uploadPhoto(file: File): Promise<string> {
    console.log('📤 [UserSync] Uploading photo...');
    try {
      const token = await getAccessToken(); // ✅ CRITICAL FIX: Use async getAccessToken
      const formData = new FormData();
      ...
      const data = await response.json();
      if (data.success && data.url) {
        console.log(`✅ [UserSync] Photo uploaded: ${data.url}`);
        return data.url;
      } else {
        throw new Error('Upload failed');
      }
  ...
  async getAllUsers(): Promise<any[]> {
    console.log('🔄 [UserSync] Getting all users (admin)...');
    try {
      const data = await apiRequest('/admin/users', { method: 'GET' });

      if (data.success && data.users) {
        console.log(`✅ [UserSync] Got ${data.users.length} users`);
        return data.users;
      } else {
  ...
  async updateUserStatus(userId: string, status: string): Promise<any> {
    console.log(`🔄 [UserSync] Updating user ${userId} to ${status}...`);
    try {
      ...
      if (data.success && data.user) {
        console.log(`✅ [UserSync] Updated user ${userId} to ${status}`);
        return data.user;
      } else {
...
export async function initializeApp(): Promise<void> {
  console.log('🚀 [SyncManager] Initializing app...');
  ...
  try {
    ...
    if (token) {
      try {
        ...
        if (currentUser) {
          console.log('🔄 [SyncManager] Loading orders for authenticated user...');
          try {
            ...
            console.log(`✅ [SyncManager] Loaded ${orders.length} orders`);
          } catch (orderError) {
    ...
    console.log('✅ [SyncManager] App initialized');
  } catch (error) {
...
export async function syncAdminData(): Promise<void> {
  console.log('🔄 [SyncManager] Syncing all admin data...');
  ...
  try {
    ...
    if (products.status === 'fulfilled') {
      console.log('✅ [SyncManager] Products synced');
    } else {
      console.error('❌ [SyncManager] Failed to sync products:', products.reason);
    }
    
    if (orders.status === 'fulfilled') {
      console.log(`✅ [SyncManager] Orders synced: ${orders.value.length}`);
    ...
    if (users.status === 'fulfilled') {
      console.log(`✅ [SyncManager] Users synced: ${users.value.length}`);
    } else {
      console.error('❌ [SyncManager] Failed to sync users:', users.reason);
    }
    
    console.log('✅ [SyncManager] Admin data sync complete');
  } catch (error) {
