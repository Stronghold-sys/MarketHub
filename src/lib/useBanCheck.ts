...
export function useBanCheck() {
  ...
  console.log('[useBanCheck] Hook called - user:', user?.id);
  ...
  useEffect(() => {
    ...
    const checkBanStatus = async () => {
      try {
        console.log(`[useBanCheck] Checking ban status for user: ${user.id}`);
        ...
        console.log(`[useBanCheck] Ban status response:`, response);
        ...
        if (response.success && response.banned && response.banData) {
          ...
          console.log(`[useBanCheck] User is not banned (banned: ${response.banned})`);
        }
      } catch (error) {
        // Silently fail - don't spam errors if endpoint not ready
        console.log('[useBanCheck] Could not check ban status:', error instanceof Error ? error.message : 'Unknown error');
      }
    };
