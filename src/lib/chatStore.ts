...
export function subscribeToRealtime() {
  if (realtimeSubscription) {
    console.log('⚠️ [Chat] Already subscribed to realtime');
    return;
  }

  console.log('🔌 [Chat] Subscribing to Supabase Realtime...');
  ...
  realtimeSubscription = supabase
    ...
    .on(
      ...
      async (payload) => {
        console.log('🔔 [Chat] Realtime event:', payload.eventType, payload.new?.key);
...
export function unsubscribeFromRealtime() {
  if (realtimeSubscription) {
    console.log('🔌 [Chat] Unsubscribing from realtime...');
...
export const chatStoreHelpers = {
  ...
  fetchConversations: async () => {
    try {
      ...
      console.log(`✅ [Chat] Fetched ${conversations.length} conversations`);
    } catch (error) {
  ...
  fetchMessages: async (conversationId: string) => {
    try {
      ...
      console.log(`✅ [Chat] Fetched ${messages.length} messages for ${conversationId}`);
    } catch (error) {
  ...
  updateConversation: async (conversationId: string, updates: Partial<Conversation>) => {
    try {
      ...
      console.log('✅ [Chat] Conversation updated:', conversationId);
