...
export async function getCartItemsBackend(): Promise<CartItemBackend[]> {
  ...
  if (!token) {
    console.log('ℹ️ [getCartItemsBackend] No auth token available');
    throw new Error('No authentication token');
  }
  
  console.log('📦 [getCartItemsBackend] Fetching cart items...');
  ...
  if (!response.ok) {
    ...
    try {
      ...
      if (response.status === 401) {
        console.log('ℹ️ [getCartItemsBackend] Unauthorized (401) - User session expired or invalid');
  ...
  const result = await response.json();
  console.log(`✅ [getCartItemsBackend] Fetched ${result.data?.length || 0} items`);
  return result.data || [];
}
...
export async function syncCartFromBackend(products: Product[]): Promise<CartItem[] | null> {
  try {
    ...
    const token = await getAuthToken();
    if (!token) {
      console.log('ℹ️ [syncCartFromBackend] No auth token, returning empty cart');
    ...
    console.log(`✅ [syncCartFromBackend] Synced ${cartItems.length} cart items from backend`);
    ...
    if (error.message === 'No authentication token') {
      console.log('ℹ️ [syncCartFromBackend] User not authenticated, returning empty cart');
      return []; // Legitimate empty cart
    } else if (error.message === 'Unauthorized') {
      console.log('⚠️ [syncCartFromBackend] Session expired or invalid token, returning null to preserve cart');
