...
export const refundStoreHelpers = {
  ...
  fetchUserRefunds: async (token: string) => {
    ...
    try {
      ...
      console.log('[RefundStore] Fetched user refunds:', data.refunds?.length || 0);
    } catch (error: any) {
  ...
  fetchAllRefunds: async (token: string) => {
    ...
    try {
      console.log('[RefundStore] ✅ Fetching all refunds...');
      console.log('[RefundStore] Token present:', token ? 'YES' : 'NO');
      ...
      try {
        ...
        if (sessionError) {
          ...
          console.log('[RefundStore] ✅ Using fresh token from Supabase session');
        } else {
          console.log('[RefundStore] ⚠️ No active session found, using provided token');
        }
      } catch (sessionFetchError) {
        console.log('[RefundStore] ⚠️ Could not get session, using provided token:', sessionFetchError);
      }
      
      const requestUrl = `${API_URL}/refunds`;
      console.log('[RefundStore] Request URL:', requestUrl);
      ...
      console.log('[RefundStore] Response status:', response.status, response.statusText);
      
      const data = await response.json();
      console.log('[RefundStore] Response data:', data);
      ...
      if (response.status === 401 || response.status === 403) {
        ...
        console.log('⚠️ [RefundStore] Authorization failed - returning empty refunds');
        return;
      }
      ...
      console.log('[RefundStore] ✅ Fetched all refunds:', data.refunds?.length || 0);
    } catch (error: any) {
  ...
  fetchRefund: async (refundId: string, token: string): Promise<Refund | null> => {
    try {
      ...
      console.log('[RefundStore] Fetched refund:', refundId);
      return data.refund;
    } catch (error: any) {
  ...
  createRefund: async (refundData: Partial<Refund>, token: string): Promise<{ success: boolean; refund?: Refund; error?: string }> => {
    ...
    try {
      ...
      console.log('[RefundStore] Refund created:', data.refund.id);
  ...
  updateRefundStatus: async (
    ...
    try {
      ...
      console.log('[RefundStore] Refund status updated:', refundId, '→', status);
      return { success: true, refund: data.refund };
    } catch (error: any) {
  ...
  uploadEvidence: async (file: File, refundId: string, token: string): Promise<{ success: boolean; evidence?: RefundEvidence; error?: string }> => {
    ...
    try {
      ...
      console.log('[RefundStore] Evidence uploaded:', refundId);
