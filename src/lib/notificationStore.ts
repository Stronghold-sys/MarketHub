...
export const notificationStoreHelpers = {
  setBanNotification: (data: BanData | null) => {
    console.log('✅ [NotificationStore] Storing ban notification:', data);
    notificationStore.setState({ banNotification: data });
  },

  clearBanNotification: () => {
    console.log('🗑️ [NotificationStore] Clearing ban notification');
    notificationStore.setState({ banNotification: null });
  },

  setDeletedNotification: (data: DeletedData | null) => {
    console.log('✅ [NotificationStore] Storing deleted notification:', data);
    notificationStore.setState({ deletedNotification: data });
  },

  clearDeletedNotification: () => {
    console.log('🗑️ [NotificationStore] Clearing deleted notification');
    notificationStore.setState({ deletedNotification: null });
  },

  addNotification: (notification: Notification) => {
    console.log('✅ [NotificationStore] Adding user notification:', notification);
  ...
  removeNotification: (id: string) => {
    console.log('🗑️ [NotificationStore] Removing user notification:', id);
    notificationStore.setState((state) => ({
  ...
  markNotificationAsRead: (id: string) => {
    console.log('✅ [NotificationStore] Marking user notification as read:', id);
    notificationStore.setState((state) => ({
  ...
  setMaintenanceMode: (mode: boolean, message: string, startTime: string | null, endTime: string | null) => {
    console.log('✅ [NotificationStore] Setting maintenance mode:', mode);
  ...
  clearMaintenanceMode: () => {
    console.log('🗑️ [NotificationStore] Clearing maintenance mode');
    notificationStore.setState({
      maintenanceMode: false,
      maintenanceMessage: '',
      maintenanceStartTime: null,
  ...
  markAllAsRead: (userId?: string) => {
    console.log('✅ [NotificationStore] Marking all notifications as read for user:', userId);
    notificationStore.setState((state) => ({
      notifications: state.notifications.map((n) => 
  ...
  deleteNotification: (id: string) => {
    console.log('🗑️ [NotificationStore] Deleting notification:', id);
    notificationStore.setState((state) => ({
      notifications: state.notifications.filter((n) => n.id !== id),
    }));
  },
