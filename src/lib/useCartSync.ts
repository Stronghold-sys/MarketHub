...
export function useCartSync() {
  ...
  console.log('🔄 [CartSync] Active - items:', items.length, 'products:', products.length);
  ...
  useEffect(() => {
    ...
    console.log('🔄 [CartSync] Syncing cart with product catalog...');
    ...
    items.forEach((cartItem) => {
      ...
      if (!latestProduct) {
        ...
        console.log(`⚠️ [CartSync] Product ${cartItem.productId} not found, removing from cart`);
      ...
      if (needsUpdate) {
        console.log(`🔄 [CartSync] Updating product data for ${cartItem.productId}`);
      ...
      if (latestProduct.stock === 0) {
        console.log(`⚠️ [CartSync] Product ${cartItem.productId} out of stock, removing`);
        removeItem(cartItem.productId);
      ...
      if (cartItem.quantity > latestProduct.stock) {
        console.log(`⚠️ [CartSync] Adjusting quantity for ${cartItem.productId}: ${cartItem.quantity} -> ${latestProduct.stock}`);
    ...
    console.log('✅ [CartSync] Sync complete');
