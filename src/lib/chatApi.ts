...
export async function getAllConversations(): Promise<Conversation[]> {
  try {
    console.log('📡 [Chat API] Fetching all conversations...');
    ...
    console.log(`✅ [Chat API] Fetched ${result.data.length} conversations`);
    return result.data;
  } catch (error: any) {
...
export async function getUserConversation(userId: string): Promise<Conversation | null> {
  try {
    console.log(`📡 [Chat API] Fetching conversation for user ${userId}...`);
    ...
    if (result.data) {
      console.log(`✅ [Chat API] Found conversation: ${result.data.id}`);
    } else {
      console.log(`ℹ️ [Chat API] No conversation found for user ${userId}`);
    }
...
export async function createConversation(conversation: Conversation): Promise<Conversation> {
  try {
    console.log('📡 [Chat API] Creating conversation:', conversation.id);
    ...
    console.log('✅ [Chat API] Conversation created:', result.data.id);
    return result.data;
  } catch (error: any) {
...
export async function updateConversation(
  ...
  try {
    console.log('📡 [Chat API] Updating conversation:', conversationId);
    ...
    console.log('✅ [Chat API] Conversation updated');
    return result.data;
  } catch (error: any) {
...
export async function getMessages(conversationId: string): Promise<Message[]> {
  try {
    ...
    console.log(`✅ [Chat API] Fetched ${result.data.length} messages`);
    return result.data;
  } catch (error: any) {
...
export async function sendMessage(message: Message, conversationId: string): Promise<Message> {
  try {
    console.log('📡 [Chat API] Sending message:', message.id);
    ...
    console.log('✅ [Chat API] Message sent:', result.data.id);
    return result.data;
  } catch (error: any) {
...
export async function markMessagesAsRead(conversationId: string): Promise<void> {
  try {
    console.log('📡 [Chat API] Marking messages as read:', conversationId);
    console.log('🌐 [Chat API] Request URL:', `${API_URL}/messages/${conversationId}/mark-read`);
